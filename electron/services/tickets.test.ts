...
describe('tickets service', () => {
  beforeEach(() => resetMemory({ projects: [PROJECT], tickets: [], counters: { p1: 0 } }));
  ...
  it('creates tickets with sequential slug-based ids and defaults', async () => {
    ...
    expect(memory.data.counters.p1).toBe(2);
  });
  ...
  it('gets a ticket or throws', async () => {
  ...
  it('updates only the given fields and bumps updated', async () => {
    await createTicket({ projectId: 'p1', title: 'A', author: 'me' });
    memory.data.tickets[0].updated = '2000-01-01';
    const t = await updateTicket('one-1', { status: 'done', due: '2026-12-31' });
    expect(t).toMatchObject({ title: 'A', status: 'done', due: '2026-12-31', updated: TODAY });
  ...
  it('deletes a ticket or throws', async () => {
    await createTicket({ projectId: 'p1', title: 'A', author: 'me' });
    await deleteTicket('one-1');
    expect(memory.data.tickets).toHaveLength(0);
    await expect(deleteTicket('one-1')).rejects.toBeInstanceOf(NotFoundError);
  });

  describe('listTickets / matchesFilter', () => {
    beforeEach(async () => {
      memory.data.projects.push({ ...PROJECT, id: 'p2', slug: 'two' });
      memory.data.counters.p2 = 0;
    ...
    it('combines filters with AND', async () => {
      ...
      expect(matchesFilter(memory.data.tickets[0], { projectId: 'p1', status: 'done' })).toBe(
        false,
      );
    });
  });
